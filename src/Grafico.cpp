...
void Grafico::plotarGrafoEntrada(){
    
    
    Gnuplot g1("Solução");
    stringstream ss;
    ss << "Grafo de entrada N: " << Graph::order  ;
    ...
    g1.set_pointsize(5).set_style("points pt 5 lc rgb \"black\"");
    
    vector< int > vertices_x(Graph::order);
    vector< int > vertices_y(Graph::order);
    for(int i = 0; i < Graph::order; i++){
        vertices_x[i] = ( i % (Graph::order - (Graph::order/2)) )+1 ;
        vertices_y[i] = ((i+1 %3)/2);
    }
    
    
    g1.plot_xy( vertices_x, vertices_y,"Vértices");
...
void Grafico::plotarGraficoDaSolucao(Solucao *s) {
	...
	stringstream ss;
	ss << "Solução da Instância " << s->d->arquivo << "\\nTempo X Atividade\\n"<< s->tempo << " X "<< s->custo ;
...
void Grafico::plotarTrandOFF(list<Solucao *> solucoes) {
    ...
    for (list<Solucao*>::iterator it = solucoes.begin(); it != solucoes.end(); it++) {
        custo.push_back((*it)->custo);
        tempo.push_back((*it)->tempo);
    }
    plotarTrandOFF(custo, tempo);
...
void Grafico::plotarTrandOFF(vector<float> custo, vector<int> tempo) {
	...
	g1.set_pointsize(3).set_style("points pt 5");
